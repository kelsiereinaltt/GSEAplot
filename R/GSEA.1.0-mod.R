...
GSEA <- function(
  ...
  for (i in 1:max.Ng) { #for(i in i:210)
    ...
    gene.set.reference[i] <- gs.line[1] #if i=210, gene.set.rrefference[i] = FAM48A @ 210
    gene.set.name <- gs.line[1] #if i=210, gene.set.name = FAM48A
    gene.set.desc <- gs.line[2] #if i=210, gene.set.desc = NA
    gene.set.tags <- vector(length = gene.set.size, mode = "character") #empty vector of length of gene.set.size
    for (j in 1:gene.set.size) {
      gene.set.tags[j] <- gs.line[j + 2] #all genes for values in temp @210, disregards first 2 (so will not include FAM48A an NA)
    }
    gene.set.reference.matrix[[i]] <- gene.set.tags
    existing.set <- is.element(gene.set.tags, gene.labels) #true or false for if genes
  ...
  names(gene.set.reference.matrix) <- gene.set.reference #assigns the names of the gene sets to the matrix
  Ng <- gs.count - 1 # we subtract one becasue in the previous for loop, at the end we add 1 so the number is 211, but we want 210
  ...
  nperm.per.call <- 100
  n.groups <- nperm %/% nperm.per.call #division
  n.rem <- nperm %% nperm.per.call #modulus
  ...
  obs.index <- order(obs.s2n, decreasing=T) #returns the indices of the vector in a sorted order
  obs.s2n   <- sort(obs.s2n, decreasing=T) # sorts the vector in an descending order
  ...
  for (i in 1:Ng) {
    ...
    gene.set2 <- match(gene.set, gene.labels) #returns position of values in gene.labels matching values in gene.set
    if (OLD.GSEA == F) {
