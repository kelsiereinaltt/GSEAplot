...
This vignette gives an introduction to the GSEAplot package. This package builds on the Gene Set Enrichment Analysis (GSEA), a computational method published by the Broad Institute. This package uses the enrichment data from the Broad Institute's analysis to create plots that are easier to understand and modify, and expands the amount of gene sets analyzed, thus increasing the amount of plots output by the analysis. In addition to improved plotting functionality, this package also introduces the capability to create your own geneset databases. The package includes all geneset databases published by the Broad Institute as well as 6 different transcription factor databases and two databases containing ligands and receptor pairs.
...
Introduction of Kadoki Ligand and Receptor Databases
...
## "non-existing Databases (change title later)"
~~explain what this could be (from another source, created/modified dataset)~~. The  instructions for how to load a "non-existing" database will be detailed in other sections of the manuscript. 

# Setting up Data for be used with GSEAplot
...
• **DELETEgs.size.threshold.max** - maximum number of gene symbols in expression data that match a given gene set. A geneset with more matching gene symbols than this will be excluded from the GSEA analysis. 
...
The parameters gs.size.threshold.min and gs.size.threshold.max determines how many gene symbols from the gene set and the expression data can match. Their default values are 2 and 9999999999 respectively, so as to allow genesets of all sizes to be analyzed (except for genesets less than the size of 2). Gs.size.threshold.max can be changed within the GSEAplots() parameters. 

```{r results="hide",warning=FALSE}
 pp= GSEAplots(input.ds.name=expr.input,
               input.cls.name=pheno.input, gene.set.input=gene.set.input,
               doc.string="GSEA_plots", nperm=1000,
...
## Function outputs
Outputs of GSEA include plots, variables gene.set.reference.matrix and gene.set.leading, reports and enrichment scores. All outputs will be saved to the working directory.
