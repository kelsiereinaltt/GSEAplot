...
```{r}
GO_mf_filename=database_key("hallmark all")
GO_mf_filename
```
...
The following script is an example of how to properly use the **GSEAplots** function. Please note that on a computer with a MacOS 10.14 (Mojave) operating system, 2.2 GHz Intel Core i7 processor, and 16 GB of memory, the run time for this function with its current parameters is approximately 3.244399 minutes.
...
```{r fig.width=4,fig.height=4}
 enrichment_TF$plots[1] #kelsie - plot most significant 
