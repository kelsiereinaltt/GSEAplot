...
To improve on the GSEA code published by the Broad Institute, additional geneset database options were included within this package. The GSEAplot function was made to be compatible with other data sets, whether the data comes from another source or is modified by the user.
Detailed below.
...
GO_mf_filename
GO_mf_filename = database_key(descriptive_names[20]) #kelsie -test this
```
...
```{r}
data(hallmark.gs)
data(C1.gs) #make this  hhallmark 
```
...
The following script is an example of how to properly use the **GSEAplots** function. Please note that on a computer with a MacOS 10.14 (Mojave) operating system, 2.2 GHz Intel Core i7 processor, and 16 GB of memory, the run time for this function with its current parameters is approximately 3.244399 minutes. kelsie NOTE.
...
## Function Outputs
Outputs of **GSEAplots** include reports, enrichment scores, plots, and the variables gene.set.reference.matrix and gene.set.leading. All outputs will be saved to the working directory.
...
The following example displays the leading gene symbols for one of gene sets:
```{r}
names(pp$gene.set.reference.matrix)[[50]]
...
custom_db=list(entry1,entry2,entry3) #kelsie - if vignette doesn't run check here
names(custom_db)=c(sets[1],sets[2],sets[3]) #and here
gene.set.input=create_geneset_db(custom_db) #and here
```
...
```{r fig.width=4,fig.height=4}
 enrichment_TF$plots[1] #kelsie - plot the most significant one
...
```{r fig.width=4,fig.height=4}
 receptor_results$plots[1] #kelsie - plot the most significant one
